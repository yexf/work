 ...
 *
 * $Id: musicout.c,v 1.2 2003/03/11 10:07:40 sps Exp $
 *
 * $Log: musicout.c,v $
 * Revision 1.2  2003/03/11 10:07:40  sps
 * committed contribution from Mike Coleman to remove certain kinds of
 * compiler warnings
 *
 * Revision 1.1.1.1  2002/11/08 16:28:14  sps
 * dist10
 *
...
/* Implementations */
/*
  A safer replacement for gets()
 */
char *ReadUserText( char *s, int size, FILE *stream )
{
   int termIdx;
   char* ret;
   ret = fgets( s, size, stream );
   if ( (ret != NULL) && ((termIdx = strlen(ret)) > 0) )
   {
      termIdx -= 1;
      if ( (termIdx >= 0) && ((ret[termIdx] == '\n') || (ret[termIdx] == '\r')) )
      {
         ret[ termIdx ] = '\0';
      }
      termIdx -= 1;
...
typedef double VE[7][HAN_SIZE];		
	...
	unsigned long     bitsPerSlot, samplesPerFrame=0;
...
/******************************************************************************/
	...
	{        /* no command line args -> interact */
	   do
	   {
		  ...
		  ReadUserText (encoded_file_name,MAX_NAME_SIZE,stdin);
	   ...
	   ReadUserText (decoded_file_name,MAX_NAME_SIZE,stdin);
...
/* JMZ 10/03/1995 Multilingual */
	   ...
	   ReadUserText(ext_bitstream_name,MAX_NAME_SIZE,stdin);
...
/*********************Ausgabe***************************************************/
fprintf(fp1, "******************** FRAME %ld *****************************\n", frameNum-1);
fprintf(fp2, "\n******************** FRAME %ld *****************************\n", frameNum-1);
fprintf(fp3, "\n******************** FRAME %ld *****************************\n", frameNum-1);
fprintf(fp4, "******************** FRAME %ld *****************************\n", frameNum-1);
fprintf(fp6, "******************** FRAME %ld *****************************\n", frameNum-1);
