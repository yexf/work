 ...
 *
 * $Id: common.c,v 1.3 2003/10/13 13:52:09 tmm01 Exp $
 *
 * $Log: common.c,v $
 * Revision 1.3  2003/10/13 13:52:09  tmm01
 *   by Marc Klein Middelink, look for: mkm
 * + adapted pick_table() to handle free format
 *
 * Revision 1.2  2003/03/11 10:07:39  sps
 * committed contribution from Mike Coleman to remove certain kinds of
 * compiler warnings
 *
 * Revision 1.1.1.1  2002/11/08 16:28:13  sps
 * dist10
 *
...
int pick_table (frame_params *fr_ps)
{
   ...
   if ((sfrq == 48 && (br_per_ch == 0 || br_per_ch >= 56)) ||         /* fixed 2003-10-13 mkm */
       (br_per_ch >= 56 && br_per_ch <= 80)) table = 0;
   else if (sfrq != 48 && (br_per_ch == 0 || br_per_ch >= 96)) table = 1;
...
unsigned int get1bit(Bit_stream *bs)
{
   ...
   {
#ifdef	PrintBitDebug
      printf ("pos: %5ld bits: %2d code: %4x val: %5d\n",
   ...
   {
      fprintf (stderr, "get1bits: bs->curpos = %4ld bs->totbits = %4ld bits\n",
...
unsigned long getbits(Bit_stream *bs, int n)
{
...
#ifdef	PrintBitDebug
      printf ("pos: %5ld bits: %2d code: %4lx val: %5ld\n",
   ...
   {
      fprintf (stderr, "getbits: n: %4d bs->curpos: %4ld bs->totbits = %4ld bits\n",
